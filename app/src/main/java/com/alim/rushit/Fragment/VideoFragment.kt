...
import com.alim.rushit.Adapter.ProfileVideoAdapter
...
class VideoFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        model.imagelink = resources.getString(R.string.link1)
        data.add(model)
        val model1 = ProfilePostModel()
        model1.description = "Second description"
        model1.imagelink = resources.getString(R.string.link2)
        data.add(model1)
        val model2 = ProfilePostModel()
        model2.description = "Third description"
        model2.imagelink = resources.getString(R.string.link3)
        data.add(model2)
        //
        val layoutManager = LinearLayoutManager(activity!!)
        recycler.layoutManager = layoutManager
        val adapter = ProfileVideoAdapter(activity!!, data)
